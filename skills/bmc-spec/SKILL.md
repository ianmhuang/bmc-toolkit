...
| `refresh [DOC] [--write] [--skip-source SOURCE]` | maintainer command: what `check` found, per version (`add`, `changed`, `confirm` with the reason: an OCP URL to find, a DMTF Work-in-Progress row); `--write` inserts the `add` entries into the catalog file at their place in publication order (a document whose version blocks the text scan cannot read is reported and left alone); `--skip-source dmtf|nvme|ocp` (repeatable) leaves that publisher's documents out. Not for answering questions |
...
| `repos [--topic T]` | the catalog's repositories, one per line, four tab-separated fields: id, held Code Trees (`commit7 provenance`, `superseded` when re-fetched) or `-`, `user checkout: path` or `-`, topics; a first line `release: L (config.toml) -> openbmc <commit7>` when a default Release is set. Vendor trees are the ids starting `aspeed-` or `nuvoton-` (their topics carry the vendor and SoC names); they are the only vendor repositories `clone` accepts |
...
| `clone REPO [--ref R \| --release L] [--force]` | bring the repository into the Library as a Code Tree (REPO not in the catalog: `https://github.com/openbmc/REPO.git` is tried, with a `note:` saying so, and `repos` lists it as `(not in catalog)` afterwards; an unlisted `aspeed-*` or `nuvoton-*` id exits 2 instead): at branch/tag/full commit R, at the Pin of OpenBMC release L (not for a vendor tree: exit 2) (the `openbmc` repository is fetched at L first, recipes only), or at the default branch. Prints `cloned <repo> <commit7> (<provenance>) -> <path>` or `held ...` when already there (no network); `--force` resolves a moving name again and prints `superseded` for the older tree of that branch, whether it was held under `--ref` or as the default branch; a plain clone also marks superseded a default-branch tree another branch left (a changed catalog `ref`). Uses the `config.toml` default Release when no flag is given, and says `release: L (from config.toml)` |
...
1. Pick the repository: `repos --topic T` (ipmi, redfish, pldm, sensors,
   ...
   A question that names a SoC vendor (Aspeed, Nuvoton), a SoC (AST2600,
   AST2700, NPCM7XX, NPCM8XX) or a vendor SDK version goes to that
   vendor's tree first (`repos --topic aspeed`, `--topic ast2600`, ...:
   `aspeed-linux`, `aspeed-u-boot`, `aspeed-openbmc`, `nuvoton-linux`,
   ...); anything else goes upstream first. When you read both and they
   differ (a driver only the vendor carries, another compatible string, a
   different register sequence), say how; one side alone is not a
   comparison. Only the catalog's vendor trees can be cloned; an unlisted
   `aspeed-*` or `nuvoton-*` id is refused, not guessed.
2. Pick the commit. The user names a release ("we ship 2.18.0", "our base
   ...
   `clone` and the reading commands then use and announce. A vendor tree
   has no Release: it is held at the vendor's default branch (its newest
   SDK branch), another SDK branch is `--ref`, `--release` exits 2 and the
   configured Release is skipped with a `note:`. Name the tree's branch
   and date in the answer, as the `cite:` line gives them. Never edit
