...
| `catalog --table [--golden FILE] [--by-family]` | the Support Level table (Markdown): family, document, access (`open (latest gated)` when the newest version differs), latest, fetch, Verified (the question ids from the Golden Questions file's `Document` column, grouped by the version they were checked on: `G13, G15 (1.3.1)`; `-` without `--golden`), known limit; `--by-family` prints one heading and table per family instead, the form kept in `docs/SUPPORT.md`. For README and support questions, not for answering a specification question |
